<template>
	<view class="search">
		<musicHead title="搜索" :icon="true" color="" :iconBlack='true'></musicHead>
		<view class="container">
			<scroll-view scroll-y="true" >
				<view class="search-search">
					<text class="iconfont icon-fangdajing"></text>
					<input type="text" value="" placeholder="搜索歌曲" v-model="searchWord" @confirm="handleToSearch(searchWord)" @input="handleToSuggest"/>
				...
				<block v-if="searchType == 1">
					...
					<view class="search-hot">
						...
						<view class="search-hot-item" v-for="(item,index) in searchHot" :key='index' @tap="handleToWord(item.searchWord)">
							...
							<text class="search-hot-count">{{ formatScore(item.score)}}</text>
						</view>
					</view>
				</block>
...
<style scoped>
	.search{padding-bottom: 15rpx;}
	.search-search{display: flex;
		align-items: center;
		height: 70rpx;
	...
	.search-hot-word view:nth-child(2){font-size: 24rpx;color: #878787; width:65vw;white-space: nowrap; overflow: hidden;text-overflow: ellipsis;}
	...
	.search-suggest-item text{color: #bdbdbd;margin-right: 28rpx;position: relative;top: 1px;}
</style>
